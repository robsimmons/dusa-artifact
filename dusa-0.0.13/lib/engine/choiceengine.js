import { TRIV_DATA, dataToString, hide } from '../datastructures/data.js';
import { DataMap } from '../datastructures/datamap.js';
import { dbToString, insertFact, listFacts, stepDb } from './forwardengine.js';
...
export function stepTreeRandomDFS(program, tree, path, stats) {
    switch (tree.type) {
        case 'leaf': {
...
export function execute(program, db, debug = false) {
    let tree = { type: 'leaf', db };
    let path = [];
    const stats = { cycles: 0, deadEnds: 0 };
    const solutions = [];
    for (;;) {
        if (tree === null)
            return { solutions, steps: stats.cycles, deadEnds: stats.deadEnds };
        if (debug) {
...
export function pathToString(tree, path) {
    return `~~~~~~~~~~~~~~
...
export function solutionsToStrings(solutions) {
